...
class ScoreboardModalController extends ModalController {
  public function generateIndicator(): string {
    ...
    if ($game->getValue() === '1') {
      ...
      $s_each = $seconds > 0 ? intval($seconds / 10) : 1;
      $now = time();
        ...
        $scoreboard_tbody .=
          '<tr>' .
            '<td style="width: 10%;" class="el--radio">' .
              '<input type="checkbox" name="fb-scoreboard-filter" id="' . htmlspecialchars($team_id) . '" value="' . htmlspecialchars($team->getName()) . '" checked />' .
              '<label class="click-effect" for="' . htmlspecialchars($team_id) . '">' .
                '<span style="' . htmlspecialchars($style) . '"></span>' .
              '</label>' .
            '</td>' .
            '<td style="width: 10%;">' . htmlspecialchars((string)$rank) . '</td>' .
            '<td style="width: 40%;">' . htmlspecialchars($team->getName()) . '</td>' .
            '<td style="width: 10%;">' . htmlspecialchars(strval($quiz)) . '</td>' .
            '<td style="width: 10%;">' . htmlspecialchars(strval($flag)) . '</td>' .
            '<td style="width: 10%;">' . htmlspecialchars(strval($base)) . '</td>' .
            '<td style="width: 10%;">' . htmlspecialchars(strval($team->getPoints())) . '</td>' .
          '</tr>';
        $rank++;
      }
    }
    ...
    return
      '<div class="fb-modal-content fb-row-container">' .
        '<div class="modal-title row-fixed">' .
          '<h4>' . htmlspecialchars(tr('Scoreboard')) . '</h4>' .
        ...
        '<div class="game-progress fb-progress-bar fb-cf row-fixed">' .
          '<span class="label label--left">[' . htmlspecialchars(tr('Start')) . ']</span>' .
          '<span class="label label--right">[' . htmlspecialchars(tr('End')) . ']</span>' .
          $indicator .
        '</div>' .
        '<div class="game-scoreboard fb-row-container">' .
          '<table class="row-fixed">' .
            '<thead>' .
              '<tr>' .
                '<th style="width: 10%;">' . htmlspecialchars(tr('Filter')) . '</th>' .
                '<th style="width: 10%;">' . htmlspecialchars(tr('Rank')) . '</th>' .
                '<th style="width: 40%;">' . htmlspecialchars(tr('Team Name')) . '</th>' .
                '<th style="width: 10%;">' . htmlspecialchars(tr('Quiz Pts')) . '</th>' .
                '<th style="width: 10%;">' . htmlspecialchars(tr('Flag Pts')) . '</th>' .
                '<th style="width: 10%;">' . htmlspecialchars(tr('Base Pts')) . '</th>' .
                '<th style="width: 10%;">' . htmlspecialchars(tr('Total Pts')) . '</th>' .
              '</tr>' .
            '</thead>' .
          '</table>' .
          '<div class="row-fluid main-data">' .
            '<table class="row-fixed">' . $scoreboard_tbody . '</table>' .
          '</div>' .
        '</div>' .
      '</div>';
  }
