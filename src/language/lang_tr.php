<?php

/* HH_IGNORE_ERROR[1002] */
$translations = array(
  'date and time format' =>
    'H:i:s D d/M/Y', //used by date() function
  //Translations for IndexController
  'Facebook CTF' =>
    'Facebook CTF',
  'Conquer the world' =>
    'Dünyayı fethet',
  'Play' =>
    'Oyna',
  'Welcome to the Facebook Capture the Flag Competition. By clicking "Play," you will be entered into the official CTF challenge. Good luck in your conquest.' =>
    'Facebook Capture the Flag yarışmasına hoşgeldiniz. "Oyna"ya tıklayarak resmi CTF görevine katılacaksınız. Fetihlerinizde iyi şanslar.',
  'Get ready for the CTF to start and access the gameboard now!' =>
    'Oyun panosuna girin ve CTF\'nin başlamasına hazırlanın!',
  'Gameboard' =>
    'Oyun Panosu',
  'Register Team' =>
    'Takım Kaydet',
  'Get ready for the CTF to start and register your team now!' =>
    'Takımınızı kaydedin ve CTF\'nin başlamasına hazırlanın!',
  'Login' =>
    'Giriş Yap',
  'Soon' =>
    'Yakında',
  'Upcoming Game' =>
    'Sıradaki Oyun',
  '_days' =>
    '_gün',
  '_hours' =>
    '_saat',
  '_minutes' =>
    '_dakika',
  '_seconds' =>
    '_saniye',
  'Official CTF Rules' =>
    'Resmi CTF Kuralları',
  'Following actions are prohibited, unless explicitly told otherwise by event Admins.' =>
    'Aşağıdaki eylemler, etkinlik Yöneticileri tarafından açık bir şekilde aksi belirtilmedikçe yasaklanmıştır.',
  'Rule' =>
    'Kural',
  'Cooperation' =>
    'İşbirliği',
  'No cooperation between teams with independent accounts. Sharing of keys or providing revealing hints to other teams is cheating, don’t do it.' =>
    'Bağımsız hesapları olan takımlar arasında işbirliği yasaktır. Diğer takımlarla anahtar paylaşmak veya onlara açıklayıcı ipuçları vermek hile sayılmaktadır, bunları yapmayın.',
  'Attacking Scoreboard' =>
    'Saldırma Skor Tablosu',
  'No attacking the competition infrastructure. If bugs or vulns are found, please alert the competition organizers immediately.' =>
    'Yarışma altyapısına saldırmak yasaktır. Eğer bug veya açıklar bulunursa lütfen yarışma organizatörlerine hemen haber verin.',
  'Sabotage' =>
    'Sabotaj',
  'Absolutely no sabotaging of other competing teams, or in any way hindering their independent progress.' =>
    'Yarışan diğer takımları sabote etmek veya onların bağımsız ilerleyişlerini herhangi bir biçimde sekteye uğratmak kesinlikle yasaktır.',
  'Bruteforcing' =>
    'Brute Force Kullanma',
  'No brute forcing of challenge flag/ keys against the scoring site.' =>
    'Skor sitesinde görev flag\'lerini/anahtarlarını brute force yöntemiyle denemek yasaktır.',
  'Denial Of Service' =>
    'Denial Of Service',
  'DoSing the CTF platform or any of the challenges is forbidden.' =>
    'CTF platformunu veya herhangi bir görevi DoS\'lamak yasaktır.',
  'Legal' =>
    'Yasal',
  'Disclaimer' =>
    'Feragatname',
  'By participating in the contest, you agree to release Facebook and its employees, and the hosting organization from any and all liability, claims or actions of any kind whatsoever for injuries, damages or losses to persons and property which may be sustained in connection with the contest. You acknowledge and agree that Facebook et al is not responsible for technical, hardware or software failures, or other errors or problems which may occur in connection with the contest.' =>
    'Yarışmaya katılmak suretiyle Facebook\'u, onun çalışanlarını ve barındırma hizmeti veren kurumu, yarışmayla bağlantılı olarak cana ve mala gelebilecek zararlar, hasarlar ve kayıplarla ilgili her türlü ve tüm yükümlülüklerden, davalardan veya işlemlerden masun tutmayı kabul etmektesiniz. Teknik, donanımsal veya yazılımsal arıza veya hatalarda, ya da yarışmayla ilişkili olarak gerçekleşebilecek başka hatalarda Facebook ve diğerlerinin sorumluluk sahibi olmadığını kabulleniyor ve onaylamaktasınız.',
  'If you have any questions about what is or is not allowed, please ask an organizer.' =>
    'Nelere izin verilip nelerin yasaklandığı hakkında herhangi bir sorunuz varsa lütfen bir organizatöre sorun.',
  'Have fun!' =>
    'İyi eğlenceler!',
  'Name' =>
    'Ad',
  'Email' =>
    'E-posta',
  'Token' =>
    'Token',
  'Team Registration' =>
    'Takım Kaydetme',
  'Team Name' =>
    'Takım Adı',
  'Password' =>
    'Şifre',
  'Choose an Emblem' =>
    'Amblem seçin',
  'or upload your own' =>
    'veya kendi yüklemenizi yükleyin',
  'Clear your custom emblem to use a default emblem.' =>
    'Varsayılan amblemi kullanmak için özel ambleminizi boşaltın.',
  'Password is too simple' =>
    'Şifre çok basit',
  'Sign Up' =>
    'Kayıt Olun',
  'Register to play Capture The Flag here. Once you have registered, you will be logged in.' =>
    'Burada Capture the Flag oynamak için kaydolun. Kayıt olduğunuzda giriş yapacaksınız.',
  'Not Available' =>
    'Mevcut Değil',
  'Team Registration will be open soon, stay tuned!' =>
    'Takım Kayıtları yakında açılacak, takipte kalın!',
  'Try Again' =>
    'Yeniden Deneyin',
  'Select' =>
    'Seç',
  'Team Login' =>
    'Takım Girişi',
  'Please login here. If you have not registered, you may do so by clicking "Sign Up" below. ' =>
    'Lütfen buradan giriş yapın. Eğer kayıt olmadıysanız aşağıdan "Kayıt Olun"a tıklayarak kayıt yaptırabilirsiniz. ',
  'Team Login will be open soon, stay tuned!' =>
    'Takım Girişi yakında açılacak, takipte kalın!',
  'ERROR' =>
    'HATA',
  'Start Over' =>
    'Baştan Başla',
  'Window is too small' =>
    'Pencere çok küçük',
  'For the best CTF experience, please make window size bigger.' =>
    'En iyi CTF deneyimi içi lütfen pencerenizin boyutlarını artırın.',
  'Thank you.' =>
    'Teşekkürler.',
  'Logout' =>
    'Çıkış Yap',
  'Registration' =>
    'Kayıt',
  'Play CTF' =>
    'CTF Oyna',
  'Rules' =>
    'Kurallar',
  //Translations for GameboardController
  'Admin' =>
    'Yönetici',
  'ADMIN' =>
    'YÖNETİCİ',
  'Navigation' =>
    'Gezinti',
  'View Mode' =>
    'Görüntüleme Modu',
  'View mode' =>
    'Görüntüleme modu',
  'Tutorial' =>
    'Öğretici',
  'Scoreboard' =>
    'Skor Tablosu',
  'You' =>
    'Sen',
  'Others' =>
    'Diğerleri',
  'All' =>
    'Herkes',
  'Leaderboard' =>
    'Liderlik Tablosu',
  'Announcements' =>
    'Duyurular',
  'Teams' =>
    'Takımlar',
  'Filter' =>
    'Filtre',
  'Activity' =>
    'Aktivite',
  'Game Clock' =>
    'Oyun Saati',
  //Translations for AdminController
  'Auto' =>
    'Otomatik',
  'All Categories' =>
    'Tüm Kategoriler',
  'Open' =>
    'Açık',
  'Tokenized' =>
    'Parçalanmış',
  'Hour' =>
    'Saat',
  'Hours' =>
    'Saat',
  'Used by' =>
    'Kullanan',
  'Used By' =>
    'Kullanan',
  'Available' =>
    'Müsait',
  'Registration Tokens' =>
    'Kayıt token\'ları',
  'Create More' =>
    'Daha Fazla Yarat',
  'Export Available' =>
    'Dışarı aktarmak mümkün',
  'Not started yet' =>
    'Henüz başlamadı',
  'Configuration' =>
    'Ayarlar',
  'Tokens' =>
    'Token\'lar',
  'Game Configuration' =>
    'Oyun Ayarları',
  'OK' =>
    'Tamam',
  'status_' =>
    'durum_',
  'On' =>
    'Açık',
  'Off' =>
    'Kapalı',
  'Player Names' =>
    'Oyuncu Adları',
  'Players Per Team' =>
    'Takım Başına Oyuncu',
  'Registration Type' =>
    'Kayıt Türü',
  'Strong Passwords' =>
    'Güçlü Şifreler',
  'Team Selection' =>
    'Takım Seçimi',
  'Game' =>
    'Oyun',
  'Scoring' =>
    'Puanlama',
  'Progressive Cycle (s)' =>
    'İlerleyici Döngü(ler)',
  'Refresh Gameboard' =>
    'Oyun Panosunu Yenile',
  'Default Bonus' =>
    'Varsayılan Bonus',
  'Bases Cycle (s)' =>
    'Temel Döngü(ler)',
  'Default Bonus Dec' =>
    'Varsayılan Bonus Dec',
  'Timer' =>
    'Süre Ölçer',
  'Server Time' =>
    'Sunucu Saati',
  'Game Duration' =>
    'Oyun Süresi',
  'Begin Time' =>
    'Başlangıç Saati',
  'Expected End Time' =>
    'Tahmini Bitiş Saati',
  'Internationalization' =>
    'Uluslararasılaşma',
  'Language' =>
    'Dil',
  'Branding' =>
    'Dağlama',
  'Custom Logo' =>
    'Özel Logo',
  'Logo' =>
    'Logo',
  'Custom Text' =>
    'Özel Metin',
  'DELETE' =>
    'SİL',
  'Delete' =>
    'Sil',
  'No Announcements' =>
    'Duyuru yok',
  'Game Controls' =>
    'Oyun Kontrolleri',
  'Write New Announcement here' =>
    'Buraya Yeni Duyuru yazın',
  'Create' =>
    'Yarat',
  'General' =>
    'Genel',
  'Back Up Database' =>
    'Veritabanını Yedekle',
  'Export Full Game' =>
    'Tüm Oyunu Dışa Aktar',
  'Import Full Game' =>
    'Tüm Oyunu Dışarıdan Al',
  'Import Teams' =>
    'Takımları Dışarıdan Al',
  'Export Teams' =>
    'Takımları Dışa Aktar',
  'Import Logos' =>
    'Logoları Dışarıdan Al',
  'Export Logos' =>
    'Logoları Dışa Aktar',
  'Import Levels' =>
    'Seviyeleri Dışarıdan Al',
  'Export Levels' =>
    'Seviyeleri Dışa Aktar',
  'Import Categories' =>
    'Kategorileri Dışarıdan Al',
  'Export Categories' =>
    'Kategorileri Dışa Aktar',
  'Levels' =>
    'Seviyeler',
  'New Quiz Level' =>
    'Yeni Quiz Seviyesi',
  'Title' =>
    'Başlık',
  'Question' =>
    'Soru',
  'Level title' =>
    'Seviye Başlığı',
  'Quiz question' =>
    'Quiz sorusu',
  'Country' =>
    'Ülke',
  'Answer' =>
    'Yanıt',
  'Points' =>
    'Puan',
  'Hint' =>
    'İpucu',
  'Hint Penalty' =>
    'İpucu Cezası',
  'EDIT' =>
    'DÜZENLE',
  'All Quiz Levels' =>
    'Tüm Quiz Seviyeleri',
  'Filter By:' =>
    'Filtrele:',
  'All Status' =>
    'Tüm Durumlar',
  'Enabled' =>
    'Etkin',
  'Disabled' =>
    'Etkin Değil',
  'Quiz Level' =>
    'Quiz Seviyesi',
  'Show Answer' =>
    'Yanıtı Göster',
  'Bonus' =>
    'Bonus',
  '-Dec' =>
    '-Dec',
  'Save' =>
    'Kaydet',
  'Quiz Management' =>
    'Quiz Yönetimi',
  'Add Quiz Level' =>
    'Quiz Seviyesi Ekle',
  'New Flag Level' =>
    'Yeni Flag Seviyesi',
  'Description' =>
    'Tanım',
  'Level description' =>
    'Seviye tanımı',
  'Category' =>
    'Kategori',
  'Flag' =>
    'Flag',
  'flag' =>
    'flag',
  'All Flag Levels' =>
    'Tüm Flag Seviyeleri',
  'New Attachment:' =>
    'Yeni Ek:',
  'Attachment' =>
    'Ek',
  'Link' =>
    'Bağlantı',
  'New Link:' =>
    'Yeni Bağlantı:',
  'Flag Level' =>
    'Flag Seviyesi',
  'Categories' =>
    'Kategoriler',
  '+ Attachment' =>
    '+ Ek',
  '+ Link' =>
    '+ Bağlantı',
  'Flags Management' =>
    'Flag Yönetimi',
  'Add Flag Level' =>
    'Tüm Flag Seviyeleri',
  'New Base Level' =>
    'Yeni Base Seviyesi',
  'Keep Points' =>
    'Kale Puanları',
  'Capture points' =>
    'Ele geçirme puanları',
  'All Base Levels' =>
    'Tüm Base Seviyeleri',
  'Base Level' =>
    'Base Seviyesi',
  'Bases Management' =>
    'Base Yönetimi',
  'Add Base Level' =>
    'Base Seviyesi Ekle',
  'New Category' =>
    'Yeni Kategori',
  'Category: ' =>
    'Kategori: ',
  'Categories Management' =>
    'Kategori Yönetimi',
  'Add Category' =>
    'Kategori Ekle',
  'All Countries' =>
    'Tüm Ülkeler',
  'In Use' =>
    'Kullanımda',
  'In use' =>
    'Kullanımda',
  'Not Used' =>
    'Kullanılmamış',
  'Yes' =>
    'Evet',
  'No' =>
    'Hayır',
  'ISO Code' =>
    'ISO Kodu',
  'Countries Management' =>
    'Ülke Yönetimi',
  'No Team Names' =>
    'Takım Adı Yok',
  'time' =>
    'süre',
  'type' =>
    'tür',
  'pts' =>
    'puan',
  'Level' =>
    'Seviye',
  'level' =>
    'seviye',
  'No Scores' =>
    'Skor Yok',
  'Attempt' =>
    'Deneme',
  'No Failures' =>
    'Başarısızlık Yok',
  'Team' =>
    'Takım',
  'team' =>
    'takım',
  'Names' =>
    'İsimler',
  'Scores' =>
    'Skorlar',
  'Failures' =>
    'Başarısızlıklar',
  'New Team' =>
    'Yeni Takım',
  'Team Logo' =>
    'Takım Logosu',
  'Selected Logo:' =>
    'Seçilen Logo:',
  'Select Logo' =>
    'Logo Seç',
  'All Teams' =>
    'Tüm Takımlar',
  'Protected' =>
    'Korunan',
  'Score' =>
    'Skor',
  'Change Password' =>
    'Şifre Değiştir',
  'Admin Level' =>
    'Yönetici Seviyesi',
  'Visibility' =>
    'Görünürlük',
  'Team Management' =>
    'Takım Yönetimi',
  'Add Team' =>
    'Takım Ekle',
  'None' =>
    'Yok',
  'Logo Name' =>
    'Logo Adı',
  'Logo Management' =>
    'Logo Yönetimi',
  'Session' =>
    'Oturum',
  'Cookie' =>
    'Çerez',
  'Creation Time' =>
    'Oluşturma Saati',
  'Last Access' =>
    'Son Erişim',
  'Last Page Access' =>
    'Son Sayfa Erişimi',
  'Data' =>
    'Veri',
  'Sessions' =>
    'Oturumlar',
  'entry' =>
    'girdi',
  'No Entries' =>
    'Girdi Yok',
  'Game Logs' =>
    'Oyun Logları',
  'Game Logs Timeline' =>
    'Oyun Logları Zaman Çizelgesi',
  'End Game' =>
    'Oyunu Sonlandır',
  'Begin Game' =>
    'Oyunu Başlat',
  'Game Admin' =>
    'Oyun Yöneticisi',
  'Controls' =>
    'Kontroller',
  'Quiz' =>
    'Quiz',
  'Flags' =>
    'Flag\'ler',
  'Bases' =>
    'Base\'ler',
  'Countries' =>
    'Ülkeler',
  'Logos' =>
    'Logolar',
  //Translations for inc/* and inc/gameboard/*
  'captured' =>
    'ele geçirildi',
  'Status' =>
    'Durum',
  'Completed' =>
    'Tamamlandı',
  'Remaining' =>
    'Kalan',
  'Start' =>
    'Başlangıç',
  'End' =>
    'Bitiş',
  'Rank' =>
    'Derece',
  'pts' =>
    'puan', //points
  'Your Rank' =>
    'Derecen',
  'Your Score' =>
    'Skorun',
  'Everyone' =>
    'Herkes',
  'Your Team' =>
    'Takımın',
  'Captured' =>
    'Ele Geçirildi',
  'Initiating' =>
    'Başlatılıyor',
  'run : > boot_sequence' =>
    'run : > boot_sequence',
  'Extracting' =>
    'Çıkarılıyor',
  //Translations for Utils.php's time_ago() function
  'just now' =>
    'az önce',
  'd' =>
    'gn', //day
  'hr' =>
    'sa', //hour
  'min' =>
    'dk', //minute
  'sec' =>
    'sn', //second
  'ds' =>
    'gn', //days
  'hrs' =>
    'sa', //hours
  'mins' =>
    'dk', //minutes
  'secs' =>
    'sn', //seconds
  'ago' =>
    'önce',
  //Translations for ModalControllers
  'begin_' =>
    'başla_',
  'Are you sure you want to kick off the game? Logs will be cleared and progressive scoreboard will start' =>
    'Oyunu başlatmak istediğinizden emin misiniz? Log\'lar temizlenecek ve ilerlemeli skor tablosu başlayacak',
  'end_' =>
    'bitir_',
  'Are you sure you want to finish the current game?' =>
    'Şu anki oyunu bitirmek istediğinizden emin misiniz?',
  'Are you sure you want to logout from the game?' =>
    'Oyundan çıkış yapmak istediğinizden emin misiniz?',
  'Saved' =>
    'Kaydedildi',
  'All changes have been successfully saved.' =>
    'Bütün değişiklikler başarıyla kaydedildi.',
  'Error' =>
    'Hata',
  'Sorry your form was not saved. Please correct the all errors and save again.' =>
    'Formunuz kaydedilmediği için üzgünüz. Lütfen tüm hataları düzeltip yeniden kaydedin.',
  'cancel_' =>
    'iptal_',
  'Are you sure you want to cancel? You have unsaved changes that will be reverted.' =>
    'İptal etmek istediğinizden emin misiniz? Geri alınacak olan kaydedilmemiş değişiklikleriniz var.',
  'choose_logo' =>
    'logo_seç',
  'captured_' =>
    'ele_geçirildi_',
  'flag_owner_' =>
    'flag_sahibi_',
  'INACTIVE' =>
    'AKTİF DEĞİL',
  'PTS' =>
    'PUAN',
  'category' =>
    'kategori',
  'capture_' =>
    'ele_geçirme_',
  'Insert your answer' =>
    'Yanıtınızı girin',
  'Request Hint' =>
    'İpucu İste',
  'Submit' =>
    'Gönder',
  'hint_' =>
    'ipucu_',
  'first_capture' =>
    'ilk_ele_geçirme',
  'completed_by' =>
    'tamamlayan',
  'scoreboard_' =>
    'skor_tablosu_',
  'filter_' =>
    'filtre_',
  'rank_' =>
    'derece_',
  'team_name_' =>
    'takım_adı_',
  'quiz_pts_' =>
    'quiz_puanı_',
  'flag_pts_' =>
    'flag_puanı_',
  'base_pts_' =>
    'base_puanı_',
  'total_pts_' =>
    'toplam_puan_',
  'team_' =>
    'takım_',
  'team_members' =>
    'takım_üyeleri',
  'base_pts' =>
    'base_puanı',
  'quiz_pts' =>
    'quiz_puanı',
  'flag_pts' =>
    'flag_puanı',
  'total_pts' =>
    'toplam_puan',
  'Tool bars are located on all edges of the gameboard. Tap a category to expand and close each tool bar.' =>
    'Araç çubukları oyun panosunun tüm kenarlarında bulunmaktadır. Bir araç çubuğunda bulunan kategorileri tıklayarak genişletebilir ve kapatabilirsiniz.',
  'Tool_Bars' =>
    'Araç_Çubukları',
  'Tap the "Game Clock" to keep track of time during gameplay. Don’t let time get the best of you.' =>
    'Oyun sırasında zamanı takip etmek için "Oyun Saati"ne tıklayın. Zamanın sizi alt etmesine izin vermeyin.',
  'Game_Clock' =>
    'Oyun_Saati',
  'Countries marked with an ' =>
    'Şu simge ile işaretli ülkeler: ',
  'are captured by you.' =>
    'sizin tarafınızdan ele geçirildi.',
  ' are owned by others.' =>
    'başkaları tarafınızdan ele geçirildi.',
  'Captures' =>
    'Ele Geçirmeler',
  'Tap Plus[+] to Zoom In. Tap Minus[-] to Zoom Out.' =>
    'Yakınlaşmak için Artı[+] işaretine tıklayın. Uzaklaşmak için Eksi[-] işaretine tıklayın.',
  'Click and Drag to move left, right, up and down.' =>
    'Sola, sağa, yukarı ve aşağı hareket etmek için tıklayıp sürükleyin.',
  'Zoom' =>
    'Yakınlaştır',
  'Tap Forward Slash [/] to activate computer commands. A list of commands can be found under "Rules".' =>
    'Bilgisayar komutlarını etkinleştirmek için Eğik Çizgi[/] simgesine tıklayın. Komut listesi "Kurallar" altında bulunabilir.',
  'Command_Line' =>
    'Komut_Satırı',
  'Click "Nav" to access main navigation links like Rules of Play, Registration, Blog, Jobs & more.' =>
    'Oyun Kuralları, Kayıt, Blog, Kariyer ve daha fazla ana yönlendirme bağlantısına ulaşmak için "Gez"e tıklayın.',
  'Track your competition by clicking "scoreboard" to access real-time game statistics and graphs.' =>
    '"skor tablosu"na tıklayarak gerçek zamanlı oyun istatistiklerine ve grafiklere erişin, rakiplerinizi takip edin.',
  'Have fun, be the best and conquer the world.' =>
    'Eğlenin, en iyisi olun ve dünyayı fethedin.',
  'Game_On' =>
    'Oyun_Başlasın',
  'tutorial_' =>
    'öğretici_',
  'Next' =>
    'İleri',
  'Skip to play' =>
    'Oyuna geç',
  'Powered By Facebook' =>
    'Facebook Tarafından Geliştiriliyor',
  'Active Directory / LDAP' =>
    'Active Directory / LDAP',
  'LDAP Server' =>
    'LDAP Sunucusu',
  'LDAP Port' =>
    'LDAP Bağlantı Noktası',
  'LDAP Domain' =>
    'LDAP Etki Alanı',
);
